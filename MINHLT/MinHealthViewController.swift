//
//  IMWHOViewController.swift
//  IMWHO
//
//  Created by madhav sharma on 10/8/20.
//  Copyright © 2020 madhav sharma. All rights reserved.
//

import UIKit

class MinHealthViewController: ViewController {

    @IBOutlet weak var thghtrcrdBtn: UIButton!
    @IBOutlet weak var socquesBtn: UIButton!
    @IBOutlet weak var thghttrialBtn: UIButton!
    @IBOutlet weak var worrygoBtn: UIButton!
    override func viewDidLoad() {
        super.viewDidLoad()
        thghtrcrdBtn.layer.cornerRadius = 12.5
        
        
        socquesBtn.layer.cornerRadius = 12.5
        
        
        thghttrialBtn.layer.cornerRadius = 12.5
        
        
        worrygoBtn.layer.cornerRadius = 12.5
        
        
        // Do any additional setup after loading the view.
    }
    
    

}
